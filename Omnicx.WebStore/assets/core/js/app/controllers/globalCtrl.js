(function () {
    ...
    globalCtrl.$inject = ['$scope', '$timeout', 'globalConfig', 'loader', '$http', 'CapturePlus', 'scriptLoader', 'BULKORDER_CONSTANTS', 'SERVICE_CONSTANTS', 'Recommendation', 'SUBSCRIPTION_CONSTANTS'];
    function globalCtrl($scope, $timeout, globalConfig, $http, loader, CapturePlus, scriptLoader, BULKORDER_CONSTANTS, SERVICE_CONSTANTS, Recommendation, SUBSCRIPTION_CONSTANTS) {
        var gm = this;
        gm.model = {};
        ...
        function updateBasketSubscriptionInfo() {
            ...
            var subscriptionItem = gm.basketResponse.lineItems.find(i => i.isSubscription);
            var userSetting = {
                subscriptionPlanId: gm.subscriptionPlan.RecordId,
                subscriptionTermId: gm.subscriptionModel.selectedTerm.Id,
                userPricingType: gm.subscriptionModel.selectedPricing
            };

            //Updated the user settings. 
            $http.post("/basket/UpdateBasketSubscriptionInfo", { basketId: gm.basketResponse.id, productId: gm.emptyGuid, userSetting: userSetting }).then(function (success) {
        ...
        function initSubscriptionPlan() {
            if (gm.basketResponse != null && gm.basketResponse.lineItems.length > 0) {
                ...
                if (subscriptionItem != null) {
                    ...
                    gm.subscriptionPlan = subscriptionItem.subscriptionUserSettings.subscriptionPlan;
                    ...
                    var noOfSubscriptionItemInCart = gm.basketResponse.lineItems.filter(i => i.isSubscription).length;
                    gm.subscriptionPlan.Terms = gm.subscriptionPlan.Terms.filter(i => i.SubscriptionTerm.Duration >= noOfSubscriptionItemInCart);
                    gm.subscriptionPlan.Terms.sort((a, b) => a.SubscriptionTerm.Duration - b.SubscriptionTerm.Duration);
                    if (gm.subscriptionPlan != null) {
                        //Extract subscription term.
                        if (subscriptionItem.subscriptionUserSettings.subscriptionTermId != null &&
                            subscriptionItem.subscriptionUserSettings.subscriptionTermId != gm.emptyGuid) {
                            //Extract user selected term from subscription line item. 
                            var selectedTerm = gm.subscriptionPlan.Terms.find(i => i.Id == subscriptionItem.subscriptionUserSettings.subscriptionTermId);
                            if (selectedTerm != null && selectedTerm != undefined) {
                                gm.subscriptionModel.selectedTerm = selectedTerm
                            } else {
                                gm.subscriptionModel.selectedTerm = gm.subscriptionPlan.Terms[0];
                            }
                        }
                        else {
                            //Extract default subscription term. 
                           
                            var selectedTerm = gm.subscriptionPlan.Terms.find(i => i.IsDefault);
                            if (selectedTerm != null && selectedTerm != undefined) {
                                gm.subscriptionModel.selectedTerm = selectedTerm;
                            } else {
                                gm.subscriptionModel.selectedTerm = gm.subscriptionPlan.Terms[0];
                        ...
                        if (subscriptionItem.subscriptionUserSettings.userPricingType != null &&
                            ...
                            gm.subscriptionModel.selectedPricing = subscriptionItem.subscriptionUserSettings.userPricingType
                        }
                        else {
                            //Default pricing preference is recurring.
                            gm.subscriptionModel.selectedPricing = SUBSCRIPTION_CONSTANTS.UserPricingType.Recurring;
                        }
        ...
        //// Select term accourding to basket count(If user have four subscription item in the basket, they cannot select less than four month if term is montly)
        //function UpdateSubscriptionTermDropDown() {
        //    var terms = [];
        //    var noOfSubscriptionItemInCart = gm.basketResponse.lineItems.filter(i => i.isSubscription).length;
        //    if (noOfSubscriptionItemInCart != null && noOfSubscriptionItemInCart != undefined) {
        //        terms = gm.subscriptionPlan.Terms.filter(i => i.SubscriptionTerm.Duration >= noOfSubscriptionItemInCart);
        //    }
        //    terms.sort((a, b) => a.SubscriptionTerm.Duration - b.SubscriptionTerm.Duration);
        //    gm.subscriptionPlan.Terms = terms;
        //    gm.subscriptionModel.selectedTerm = gm.subscriptionPlan.Terms[0];
        //    updateBasketSubscriptionInfo();
        //}
    };
