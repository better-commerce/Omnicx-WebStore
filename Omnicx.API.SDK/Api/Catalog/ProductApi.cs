...
namespace Omnicx.API.SDK.Api.Catalog
{
    public class ProductApi : ApiBase, IProductApi
    {
        ...
        {           
            ...
            var key = string.Format(CacheKeys.PRODUCT_MODEL_BY_ID, id, !string.IsNullOrWhiteSpace(sessionContext.LangCulture) ? sessionContext.LangCulture: sessionContext.CurrentSiteConfig?.RegionalSettings?.DefaultLanguageCulture, sessionContext.CurrentUser?.CompanyId,Utils.GetBrowserInfo().IsMobileDevice, sessionContext.CurrencyCode);
        ...
        {                   
            var sessionContext= DependencyResolver.Current.GetService<ISessionContext>();
            var key = string.Format(CacheKeys.PRODUCT_MODEL_BY_SLUG, slug, !string.IsNullOrWhiteSpace(sessionContext.LangCulture) ? sessionContext.LangCulture: sessionContext.CurrentSiteConfig?.RegionalSettings?.DefaultLanguageCulture, sessionContext.CurrentUser?.CompanyId, Utils.GetBrowserInfo().IsMobileDevice, sessionContext.CurrencyCode);
